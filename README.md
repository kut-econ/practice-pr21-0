...
画面が切り替わったら、`New`という緑色のボタンがあると思うので、こちらをクリックしてください。作成するリポジトリの設定を入力する画面になります。以下のように設定してください。
...
まず、`README.md`を次のように編集してコミットし、リモートにプッシュしてください。
...
この状態で`git log`して、 `HEAD`がブランチ`new`を指していることを確認しましょう。リモートのほうは、まだ`1 branch`となっていて、`master`のみが存在することに注意してください。

また、次のように入力して、`new`の上流ブランチが未設定であることを確認してください。
...
GitHubのサイトで、ブランチが一つだけになったことを確認してください。
