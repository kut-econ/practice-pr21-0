...
とします。`revert`が打ち消したいコミットを指定するのに対し、`reset`は取り戻したいコミットを指定することに注意してください。`--soft`オプションでは、変更はステージングするまえの状態になります。ステージングエリアももとに戻してしまいたい、`--mixed`にします。作業ディレクトリもステージングエリアも元に戻す、つまり何もかもなかったことにするには、`--hard`を指定します。
